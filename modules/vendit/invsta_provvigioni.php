...
if (isset($_SESSION['print_request'])) {
   ...
   if ($request['di'] > 0) {
      ...
      $url = ($request['sc'] ? "stampa_provvigioni_compatta.php" : "stampa_provvigioni.php");
      echo "setTimeout(\"window.location='$url?id_agente=" . $request['id_agente'] . "&datini=" . $request['di'] . "&datfin=" . $request['df'] . "'\",1000)\n";
