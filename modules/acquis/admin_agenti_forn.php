...
//    // fine rigo input
  ...
  // Se viene inviata la richiesta di conferma rigo
   ...
   if ((!empty($form['in_cod_articolo']) || $form['in_cod_catmer'] > 0) && $form['in_percentuale'] > 0) {
    ...
    if (isset($_POST['ins'])) {
       ...
       $fornitore_exist = gaz_dbi_get_row($gTables['agenti_forn'],'id_fornitore',$form['id_fornitore']);
       if (!empty($fornitore_exist) && $fornitore_exist['id_agente'] != $form['id_agente']) { // il fornitore � gi� un agente (ma non ha lo stesso id)
             $msg .= "16+";
       }
       if ($toDo == 'insert') {
          $agente_exist = gaz_dbi_get_row($gTables['agenti_forn'],'id_agente',$form['id_agente']);
  ...
  if (isset($_POST['del'])) {
    ...
    $agenti = gaz_dbi_get_row($gTables['agenti_forn'],'id_agente',$form['id_agente']);
    $anagrafica = new Anagrafica();
    ...
    // fine rigo input
    $form['cosear'] = '';
    $rs_ultimo_agente = gaz_dbi_dyn_query("id_agente", $gTables['agenti_forn'], 1,"id_agente DESC",0,1);
